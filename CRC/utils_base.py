...
def parse_example(example_proto):
    ...
    features.pop('id')
    features['label'] = tf.one_hot(features['label'], 2)
...
def make_dataset(file_path, batch_size,training):
    ...
    if training == 1:
        dataset = dataset.shuffle(80000).repeat()
...
def make_datasets(train_data_path, val_data_path, test_data_path,batch_size=32):
    ...
    return make_dataset(train_data_path, batch_size, 1), make_dataset(val_data_path, batch_size, 2), \
