...
if len(sys.argv)>=3:
    ...
    base_path = '/media/disk6/pathology_data_process_test6/pingkuang/'
 #   input_real_file='/media/disk4/pathology_data_process/xiangya-real.txt'
    input_real_file = '/media/disk4/pathology_data_process/pingkuang/pingkuang-real.txt'

input_predict_file=base_path+'heatmap_3x3/cluster_value_2x2'
output_file=base_path+'heatmap_3x3/predict-real-by-cluster_2x2'
output_statistics_file=base_path+'heatmap_3x3/statistics-by-cluster_2x2'     #statistics results
#input_predict_file='/media/disk6/pathology_data_process/group_cluster_value_3x3'
#output_file='/media/disk6/pathology_data_process/all-predict-real-by-cluster_3x3'
...
for line in real_lines:
    ...
    for line2 in reversed(predict_lines):
        ...
        if may_id!=id:
            ...
            info.append('%s %6s %8s   %8d   %s'%(id,pos_patches,total_patches,int(cluster_patches),label))
...
if len(predict_lines)!=0:
    ...
    for line in predict_lines:
        ...
        one_info='%s %6s %8s   %8d   %s'%(id,pos_patches,total_patches,int(cluster_patches),str(1))
...
for line in lines:
    ...
    if parts[-1]=='?':
        posy.append(int(parts[-2]))        #real_label

        if int(parts[-2]) == 0:
           posy_scores.append(np.min([float(parts[-3])/(2*threshold),1.0]))
           wrong_neg_count+=1
           neg_count+=1
        else:
            posy_scores.append(np.min([float(parts[-3]) / (2 * threshold), 1.0]))
        ...
        posy.append(int(parts[-1]))      #real label

        if int(parts[-1]) == 1:  #right predict label
